...
int displayCol=0;
uint16_t displayMem[R_SIZE * C_SIZE];
int displayClear = 0;
...
void scrollScreen() {
	...
	for (i = 0; i < R_SIZE * (C_SIZE - 1); i++) {
		pos = i * 2;
		data = displayMem[i+R_SIZE];
		asm volatile("movw %0, (%1)"::"r"(data),"r"(pos+0xb8000));
	}
	data = 0 | (0x0c << 8);
	for (i =R_SIZE * (C_SIZE - 1); i < R_SIZE * C_SIZE; i++) {
		pos = i * 2;
		asm volatile("movw %0, (%1)"::"r"(data),"r"(pos+0xb8000));
	}
}
