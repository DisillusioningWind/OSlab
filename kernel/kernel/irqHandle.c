...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0)
	{
		...
		ret = stringChrR(str, '/', &nameIndex);
		if(ret==-1){
			sf->eax = -1;
			return;
		}
		nameIndex++;
		stringCpy(str + nameIndex, fileName, strLen - nameIndex);
		str[nameIndex] = '\0';
		ret = readInode(&sBlock, gDesc, &fatherInode, &fatherInodeOffset, str);
		if(ret != 0 || fatherInode.type != DIRECTORY_TYPE){
			sf->eax = -1;
			return;
		}

		if ((sf->edx & O_DIRECTORY) == 0)
		{
			// TODO: Open4
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	while(sz){
		...
		if(sz <= j){
			...
			MemCpy((uint8_t *)(str + stroff), (uint8_t *)(buffer + remainder), j);
			writeBlock(&sBlock, &inode, quotient, buffer);
			sz -= j;
			remainder = 0;
			quotient++;
...
void syscallReadFile(struct StackFrame *sf) {
	...
	while (sz)
	{
		...
		j = sBlock.blockSize - remainder;
		if (sz <= j){
			MemCpy((uint8_t *)(buffer + remainder), (uint8_t *)(str + stroff), sz);
			sz = 0;
		}
		else{
			MemCpy((uint8_t *)(buffer + remainder), (uint8_t *)(str + stroff), j);
			sz -= j;
			remainder = 0;
			quotient++;
			stroff += j;
